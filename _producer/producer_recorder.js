...
// R2 Configuration for recordings
const R2_CONFIG = {
    // Update this with your actual R2 public domain
    publicUrl: 'https://pub-ac0e1b260ca5415cbc7e0b3d8a3e6ca8.r2.dev', // or your custom domain
    bucket: 'vod',
...
function setupAblyEventListeners() {
    ...
    ablyChannel.subscribe('stream-registered', (msg) => {
        const data = msg.data;
        log(`[ABLY EVENT] Stream registered: ${data.streamId} (type: ${data.type})`);
        ...
        if (data.type === 'recorder') {
            log(`Mac recorder connected: ${data.streamId}`);
            data.isRecorder = true;
            data.streamName = data.streamName || `${data.streamId}'s Mac Recorder`;
        }
    ...
    // Listen for recording progress from Mac recorders
    ablyChannel.subscribe('recording-progress', (msg) => {
        const data = msg.data;
        if (data.type === 'hls-upload-progress') {
            log(`Upload progress for ${data.streamId}: ${Math.round(data.progress * 100)}%`);
...
function handleStreamStatus(data) {
    ...
    const stream = streams.get(data.streamId);
    const isRecorder = stream && (stream.type === 'recorder' || stream.isRecorder);

    // Handle recorder-specific statuses
    if (isRecorder) {
        const safeId = sanitizeId(data.streamId);
        if (data.status === 'recording' || data.status === 'streaming') {
            log(`Mac recorder ${data.streamId} started recording`);
            // Update recorder buttons
            const startBtn = document.getElementById(`record-start-${safeId}`);
            const stopBtn = document.getElementById(`record-stop-${safeId}`);
            if (startBtn && stopBtn) {
                startBtn.disabled = true;
                stopBtn.disabled = false;
            }
        } else if (data.status === 'stopped') {
            log(`Mac recorder ${data.streamId} stopped recording`);
            // Update recorder buttons
            const startBtn = document.getElementById(`record-start-${safeId}`);
            const stopBtn = document.getElementById(`record-stop-${safeId}`);
            if (startBtn && stopBtn) {
...
function createStreamCard(stream, streamId) {
    ...
    card.className = 'video-card' + (isRecorder ? ' mac-recorder' : '');
    card.id = `stream-${safeId}`;
    card.setAttribute('data-stream-id', streamId); // Store original ID
    
    // Different controls for recorders vs regular streams
    const controlsHtml = isRecorder ? `
        <div class="video-controls">
            <button class="control-btn record-start" id="record-start-${safeId}" onclick="startMacRecording('${streamId}')">
                <i data-lucide="circle"></i>
                Start Recording
            </button>
            <button class="control-btn record-stop" id="record-stop-${safeId}" onclick="stopMacRecording('${streamId}')" disabled>
            ...
            <button class="control-btn copy-recording-link" onclick="copyRecordingLink('${stream.room}', '${streamId}')">
                <i data-lucide="link"></i>
                Copy Link
            </button>
            <button class="control-btn end" id="close-${safeId}" onclick="closeSession('${streamId}')" title="Disconnect recorder">
    ...
    card.innerHTML = `
        ...
        ${isRecorder && (stream.status === 'recording' || stream.status === 'streaming') ? `
            <div class="recording-progress" id="recording-progress-${safeId}">
                <div class="progress-bar">
                    <div class="progress-fill" id="progress-${safeId}" style="width: 0%"></div>
                </div>
                <span class="progress-text" id="progress-text-${safeId}">Recording...</span>
            </div>
        ` : ''}
    `;
    return card;
...
// Mac Recorder Control Functions
function startMacRecording(streamId) {
    log(`Starting Mac recording for ${streamId}`);
    ablyChannel.publish('producer-command', {
    ...
    const progressDiv = document.getElementById(`recording-progress-${safeId}`);
    if (progressDiv) {
...
function stopMacRecording(streamId) {
    log(`Stopping Mac recording for ${streamId}`);
    ablyChannel.publish('producer-command', {
...
function viewRecording(room, participantId) {
    const url = R2_CONFIG.getRecordingUrl(room, participantId);
    
    // Open simple HLS player
    const playerHtml = `
        <!DOCTYPE html>
        <html>
        <head>
            <title>Recording Playback - ${participantId}</title>
            <script src="https://cdn.jsdelivr.net/npm/hls.js@1.4.12/dist/hls.min.js"></script>
            <style>
                body { margin: 0; background: #000; display: flex; align-items: center; justify-content: center; height: 100vh; }
                video { width: 90%; max-width: 1200px; height: auto; }
                .error { color: white; font-family: Arial; text-align: center; padding: 20px; }
            </style>
        </head>
        <body>
            ...
            <script>
                ...
                if (Hls.isSupported()) {
                    ...
                    hls.on(Hls.Events.ERROR, (event, data) => {
                        ...
                        if (data.fatal) {
                            document.body.innerHTML = '<div class="error">Error loading recording. It may still be processing.</div>';
                        }
                    });
...
function copyRecordingLink(room, participantId) {
    ...
    navigator.clipboard.writeText(url).then(() => {
        log(`Recording URL copied: ${url}`);
        // Visual feedback
        const msg = document.createElement('div');
        msg.style.cssText = 'position: fixed; top: 20px; right: 20px; background: var(--accent-green); color: white; padding: 10px 20px; border-radius: 6px; z-index: 10000;';
        msg.textContent = 'Recording URL copied!';
        document.body.appendChild(msg);
        setTimeout(() => msg.remove(), 3000);
    }).catch(err => {
        log(`Failed to copy recording URL: ${err}`);
        alert(`Recording URL: ${url}`);
    });
}

// Make recorder functions globally accessible
